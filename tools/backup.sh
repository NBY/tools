...
BACKUP[0]="/root/docker/build"
BACKUP[1]="/root/docker/conf"
BACKUP[2]="/root/docker/mysql"
BACKUP[3]="/root/docker/www"
BACKUP[4]="/root/docker/docker-compose.yml"
...
mysql_backup() {
    if [ -z ${MYSQL_ROOT_PASSWORD} ]; then
        ...
        mysql -h 127.0.0.1 -u root -p"${MYSQL_ROOT_PASSWORD}" 2>/dev/null <<EOF
exit
EOF
        if [ $? -ne 0 ]; then
        ...
        if [ "${MYSQL_DATABASE_NAME[*]}" == "" ]; then
            mysqldump -h 127.0.0.1 -u root -p"${MYSQL_ROOT_PASSWORD}" --all-databases > "${SQLFILE}" 2>/dev/null
            if [ $? -ne 0 ]; then
            ...
            do
                ...
                mysqldump -h 127.0.0.1 -u root -p"${MYSQL_ROOT_PASSWORD}" ${db} > "${DBFILE}" 2>/dev/null
                if [ $? -ne 0 ]; then
...
gdrive_upload() {
    if ${GDRIVE_COMMAND}; then
        ...
        skicka -quiet upload ${OUT_FILE} web.backup >> ${LOGFILE}
        if [ $? -ne 0 ]; then
