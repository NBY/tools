...
# Upload to server flag (true: upload, false: not upload)
SCP_FLG=true

# SCP server
# OPTIONAL: If you want upload to FTP server, enter the Hostname or IP address below
SCP_HOST="jp1.acger.info"
SCP_LOGFILE="/root/backup/scp.log"
########## END OF CONFIG ##########
...
# Tranferring backup file to scp server
scp_upload() {
    if ${SCP_FLG}; then
	script -q -a ${SCP_LOGFILE} -c "scp ${OUT_FILE} root@${SCP_HOST}:/root/backup/"    
        log "Tranferring backup file to scp server completed"
    fi
}
